...
function AgentRegistrationForm({ onComplete, initialData, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        setMessage({ type: 'success', text: 'Agent updated successfully! (முகவர் விவரங்கள் புதுப்பிக்கப்பட்டது)' });
      } else {
        await apiPost('/users/', payload);
        setMessage({ type: 'success', text: 'Agent registered successfully with KYC details!' });
      }
      ...
      setMessage({ type: 'error', text: isEdit ? 'Error updating agent.' : 'Error registering agent. Username might already exist.' });
      console.error('Error:', error);
    }
  };

  return (
    <div className="card" style={{ borderTop: `4px solid ${isEdit ? '#10b981' : 'var(--primary-color)'}` }}>
      <h2 style={{ fontSize: '1.25rem', marginBottom: '1.5rem' }}>
        {isEdit ? 'Edit Agent Details (முகவர் விவரங்களை மாற்றுக)' : 'Register New Agent (முகவர் பதிவு)'}
      </h2>
      {message.text && (
        <div style={{ 
          padding: '1rem', 
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-grid" style={{ marginBottom: '1.25rem' }}>
          ...
          <div className="form-field">
            <label>Password (கடவுச்சொல்) {isEdit && '(புதிய கடவுச்சொல் மாற்ற வேண்டுமென்றால் மட்டும்)'}</label>
        ...
        <div className="form-grid" style={{ marginBottom: '1.25rem' }}>
          <div className="form-field">
            ...
            <input type="text" name="full_name" value={formData.full_name} onChange={handleInputChange} required placeholder="Enter agent's full name" />
          </div>
          <div className="form-field">
        ...
        <div className="form-actions">
          <button type="submit" style={{ padding: '1rem', flex: 2, fontWeight: 'bold', backgroundColor: isEdit ? '#10b981' : 'var(--primary-color)' }}>
            {isEdit ? 'Update Agent (புதுப்பிக்கவும்)' : 'Register Agent Now'}
          </button>
          {isEdit && (
